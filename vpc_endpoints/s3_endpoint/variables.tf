...
// collect s3 bucket arns
variable "s3_bucket_arns" {
    description = <<-EOT
        List of S3 bucket ARNs that the S3 VPC Interface Endpoint should allow access to.

        Purpose:
        - Restrict S3 endpoint access to specific buckets for security
        - Ensure only authorized buckets are accessible via the endpoint

        Requirements:
        - At least 1 S3 bucket ARN required
        - Buckets must exist in the same AWS account or be accessible via cross-account policies

        Example:
        s3_bucket_arns = ["arn:aws:s3:::my-bucket-1", "arn:aws:s3:::my-bucket-2"]

        Typical Use Cases:
        - EC2 instances accessing application data stored in specific S3 buckets
        - Lambda functions processing files from designated S3 buckets
        - ECS tasks retrieving assets from authorized S3 buckets
    EOT
    type        = list(string)
