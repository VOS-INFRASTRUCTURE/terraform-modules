...
This module deploys a production-ready EC2 instance with Docker pre-installed on ARM64/Graviton architecture (Ubuntu 24.04). ARM64 port of `ec2-x86-docker` — same features, same shape, retargeted architecture. It includes comprehensive security features, CloudWatch monitoring, and automated EBS snapshots.
...
- ✅ **Cost Optimized**: Graviton (`t4g`/`m7g`/`r7g` families) typically 15–20% cheaper than the x86 equivalent at the same specs
...
module "docker_server" {
  source = "../../ec2-servers/ec2-arm-docker"
  ...
  # Use defaults: t4g.medium, 20GB storage, CloudWatch enabled
}
```
...
module "docker_prod_server" {
  ...
  # Instance configuration
  instance_type = "t4g.large"  # 2 vCPU, 8 GB RAM
...
|------|-------------|---------|
| `base_name` | Base name for resources | `"docker-host"` |
| `ami_id` | Ubuntu 24.04 ARM64 AMI ID — leave empty to auto-resolve (see `data.tf`) | `""` |
| `instance_type` | EC2 instance type (Graviton — `t4g`/`m7g`/`r7g` families) | `"t4g.medium"` |
...
```hcl
module.docker_server.server  # Output object containing all server details
```
...
### Graviton (ARM64) — Best Value

| Instance Type | vCPU | RAM | Network | Use Case |
|--------------|------|-----|---------|----------|
| `t4g.micro` | 2 | 1 GB | Up to 5 Gbps | Very light workloads |
| `t4g.small` | 2 | 2 GB | Up to 5 Gbps | Light applications |
| `t4g.medium` | 2 | 4 GB | Up to 5 Gbps | **Default** — General purpose |
| `t4g.large` | 2 | 8 GB | Up to 5 Gbps | Memory-intensive apps |
| `t4g.xlarge` | 4 | 16 GB | Up to 5 Gbps | High-performance apps |

See `documentations/ComputePlan.md` (repo root) for `c6g`/`r6g` compute/memory-optimized options and estimated `eu-west-2` monthly costs.

**Requirement**: every container image and any compiled/binary dependency used on this instance must have an ARM64 (`arm64`/`aarch64`) build available.
...
Docker is pre-installed and ready to use — `docker run` automatically pulls the `arm64` image variant for any multi-arch image (most official images on Docker Hub are multi-arch):
...
# Run containers (pulls arm64 variant automatically for multi-arch images)
...
If a required image has no `arm64` build, that's a hard blocker for this module — confirm image availability before choosing ARM64 over `ec2-x86-docker`.
...
resource "aws_cloudwatch_metric_alarm" "high_memory" {
  ...
  dimensions = {
    InstanceId = module.docker_server.server.instance.id
  }
}
```
...
### Issue: Image pull fails with "no matching manifest for linux/arm64"

The image doesn't publish an ARM64 build. Either find an ARM64-compatible alternative, build your own multi-arch image, or use `ec2-x86-docker` instead for that workload.
...
|-----------|--------------|-------------------------|
| **Compute** | t4g.medium (2 vCPU, 4 GB) | ~$25 |
...
- Graviton (`t4g`) is already ~15–20% cheaper than the equivalent x86 instance
...
- [ ] Confirm every required container image has an ARM64 build
...
- [Session Manager Guide](../ec2-x86-docker/documentations/SessionManager.md)
- [CloudWatch Logs](../ec2-x86-docker/documentations/CLOUDWATCH_LOGS.md)
- [Snapshots Overview](../ec2-x86-docker/documentations/SNAPSHOTS.md)
- [AWS Systems Manager](https://docs.aws.amazon.com/systems-manager/latest/userguide/session-manager.html)

---

**Module ready for production use with Docker pre-installed on Graviton!** 🐳
